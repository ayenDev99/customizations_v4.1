...
prismApp.service('PrescriptionService', ['$http', 'ModelService', function ($http, ModelService) {
    ...
    this.savePrescriptions = function (prscrptDtls, document_sid, customer_sid, selectedPrescriptions) {
        // Define your request data here
            ...
            var columnNameCounter = ["udf1string", "udf2string", "udf3string", "udf4string", "udf5string", "udf6string", "udf7string", "udf8string", "udf9string", "udf10string"];

            // Loop through selectedPrescriptions and append columnNameCounter values
            for (var i = 0; i < selectedPrescriptions.length; i++) {
                var prescription = selectedPrescriptions[i];
                var columnName = columnNameCounter[i];
            
                // Loop through InventoryItems and add the columnNameCounter property to each object
                for (var j = 0; j < requestData.data[0].InventoryItems.length; j++) {
                    requestData.data[0].InventoryItems[j][columnName] = prescription;
                }
            }
    ...
    this.getDcs = function() {
        return $http.get('/plugins/PLPrescriptionUploadPrint/getDcs.php', {
            params: {
                sbs_sid: session.subsidiarysid,
                dcs_code: config_dcscode
            }
        });
    }
...
prismApp.factory('DataServiceUpload', function($http) {
    ...
    return {
        uploadImage: function(customerId) {
            ...
            var btnName = document.getElementById("savePrescriptionBtn");

            var data = []

            if (imageFiles.length === 0 && btnName.innerHTML === "Save") {
                alert("Please select one or more images.");
                // Return a failure response
                return Promise.reject({"Failed":true,"fileName":["viber_image_2023-03-29_14-26-10-641.jpg"]});
            }

            if (imageFiles.length === 0 && btnName.innerHTML === "Update") {
                // Return a success response
                return Promise.resolve({"success":true,"fileName":["viber_image_2023-03-29_14-26-10-641.jpg"]});
            }
