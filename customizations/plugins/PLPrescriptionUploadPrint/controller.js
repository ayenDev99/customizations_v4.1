...
prismApp.controller('prescriptionController', ["$scope", "$uibModal", "PrescriptionService", function($scope, $uibModal, PrescriptionService) {
    ...
    $scope.openModal = function() {
		...
		PrescriptionService.getDcs()
		.then(function(response) {
			config_dcssid = response.data[0].sid;
			...
			{	
				if(data.success)
				{
					ModelService.get('Inventory',{sid:prescription.sid}).then(function(data){
						...
						var fileName = imageUploadInput.files;
						if(fileName.length > 0){
							fileName = imageUploadInput.files[0].name; 
...
function getDocumentSid() {
    ...
    // Use regular expression to match the document SID part
    const regex = /\/docs\/(\d+)/;
    const match = url.match(regex);

    if (match) {
        return match[1]; // The first capture group contains the document SID
    } else {
        return null; // Return null if the document SID is not found
    }
}
