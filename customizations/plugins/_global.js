				    	...
				    	success: function(data) {
					        ...
					        if (mode == 'print') {
						        ...
						        generateSortableData(dataBase64, base64, 'xzout').then(sortable => {
									...
									ModelService.get('Store',{sid: result.store_sid}).then(function(dataStore) {
										var params = {
											created_dateTime: result.period_end,
											storeSid: result.store_sid,
											workstation: result.workstation_sid,
											workstationNo: result.workstation_number,
											contain: contain,
											printtype: printType, 
			        						sid: PCONTROLSID, 
...
function generateSortableData(payloads, base64, type) {
	return new Promise(function(resolve, reject) {
		...
		angular.forEach(payloads, function(val, key) {
		    ...
		    var bodies = angular.element(htmldata);
		    // console.log($($.parseHTML(htmldata)).html());
		    bodies.each(function(index,element) {

		    	if ($(element).hasClass('page')) {
			    	var body = bodies.find($(element).attr('id'));
				    divdata = body.prevObject;
				    parsediv = $(element).find('div');
				    // console.log($(element));
				    // for (var i = 2; i<=parsediv.length; i+=4) {

				    //     var body1 = angular.element(parsediv.prevObject[i]).find('#'+$(element).attr('id'));
				    //     console.log($(element).attr('id'));
				    //     console.log(body1);
				    //     divdata1 = body1.prevObject[0].innerHTML;
				        parsediv1 = $(element).find('div');
				        var j = 0;
				        var arrData = [];
				        for (j = 0; j < parsediv1.length; j++) {

				            if (parsediv1[j] !== undefined ){

				                var y = parseInt((parsediv1[j].style.top).replace("px",""));
				                var x = parseInt((parsediv1[j].style.left).replace("px",""));
				                ...
				                arrData[j]['x']= x;
				                arrData[j]['classname']= {};
				                arrData[j]['classname']= (parsediv1[j]).className;
				                arrData[j]['data']= {};
				                arrData[j]['data'] = ((parsediv1[j]).innerText);
				            }
				        ...
				        sortable.push({content: content, print_type: val.print_type, width: parseInt(($(element)[0].style.width).replace("px", ""))});
				    // }
			    }
			});
