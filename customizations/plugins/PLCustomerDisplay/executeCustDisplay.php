...
if ($_SERVER['REQUEST_METHOD'] == 'POST'):
   ...
   $cd_packaged_item_display     = $request->cd_packaged_item_display;
   $item     = $request->item;
   $fee      = $request->fee;
...
// echo $temp_name;
    if (count($item) > 0):
       foreach ($item as $a) {
            $item_show = 1;
            if ($cd_packaged_item_display == "all") {

            } else {
               if ($cd_packaged_item_display == "package_only") {
                  if ($a->item_kit_flag == '5') {
                     $item_show = 0;
                  } else {
                     $item_show = 1;
                  }
               ...
               if ($cd_packaged_item_display == "component_only") {
                  if ($a->item_kit_flag == '3') {
                     $item_show = 0;
                  } else {
                     $item_show = 1;
                  }
           ...
           fwrite($file, $a->item_sid . '|' . $a->item_desc . '|' . $a->item_qty . '|' . $a->item_price . '|' . $a->item_ext_price . '|' . $a->item_alu . '|' . $a->record_flag . '|' . $a->item_discount_amt . '|' . $item_show . "\r\n");
       }
    endif;
